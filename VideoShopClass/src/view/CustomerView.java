...
{
	JFrame frm;
	JTextField	tfCustName, tfCustTel,  tfCustTelAid,tfCustEmail, tfCustAddr ;
	...
	public void registCustomer(){
		...
		Customer dao = new Customer(name,addr,email,te                              l,telaid);
		// 2. 1값들을 Customer 클래스의 멤버로지정		
		try {
			model.insertCustomer(dao);
		} catch (Exception e) {
	...
	public void updateCustomer(){
				...
				Customer dao = new Customer(name,tel1,tel2,Email,adr);
				
				try {
					model.updateCustomer(dao);
					} catch (Exception e) {
						e.printStackTrace();
					}				
				//각 필드 초기화
